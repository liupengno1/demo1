...
public class ProjectGenerator {
	...
	@Test public void build() throws Exception {
		// 修改PNAME的项目名称
		String appName = System.getProperty("PNAME", "liup");
...
//						return FilenameUtils.getBaseName(file.getName()).toLowerCase().contains("yilian");
						return file.getName().toLowerCase().contains("liup");
//						return true;
					}
					...
					public boolean accept(File dir, String name) {
						return true;
//						return FilenameUtils.getBaseName(dir.getName()).toLowerCase().contains("yilian");
					}
...
//		sets.stream().forEach(e -> System.out.println(e));
		...
		for(File file : sets){
			if(file.getName().toLowerCase().contains("liup")){
				out.println(file.getAbsolutePath());
				if(file.isFile()){
					String origName = file.getName();
					String newName = StringUtils.replace(origName, "Liup", StringUtils.capitalize(appName));
					file.renameTo(new File(FilenameUtils.getFullPath(file.getAbsolutePath()) + newName));
				}else{
					if(FilenameUtils.getBaseName(file.getName()).contains("liup")){
						String origName = file.getName();
						String newName = StringUtils.replace(origName, "liup", appName);
						file.renameTo(new File(FilenameUtils.getFullPath(file.getAbsolutePath()) + newName));
					}
				}
			}
...
//		files.stream().forEach(e -> System.out.println(e));
		for(File file : files){
			if(file.isFile() && StringUtils.isNotBlank(FilenameUtils.getExtension(file.getName()))){
				...
				content = StringUtils.replace(content, "liup", appName);
				content = StringUtils.replace(content, "liup", StringUtils.capitalize(appName));
